...
$squery = "SELECT item.item_name, item.item_description, category.category_name, item.post_date,
		  ...
		  WHERE item.item_name like '%".$search."%' OR item.item_description like '%".$search."%'";
...
{
if(isset($_GET['as_submit']))
{

$asname=$_GET['asname'];
$asdesc=$_GET['asdesc'];
$ascategory=$_GET['ascategory'];
$asprice=$_GET['asprice'];
...
$squery = "SELECT item.item_name, item.item_description, category.category_name, item.post_date,
		  ...
		  INNER JOIN category ON item.category_id=category.category_id
		  INNER JOIN user ON item.user_id=user.user_id
		  WHERE item.item_name like '%".$asname."%' OR item.item_description like '%".$asdesc."%'
		  OR item.category_id='".$ascategory."'";
}
else
{
$squery = "SELECT item.item_name, item.item_description, category.category_name, item.post_date,
		  ...
		  INNER JOIN user ON item.user_id=user.user_id
		  WHERE item.item_name like '%".$asname."%' OR item.item_description like '%".$asdesc."%'
		  OR item.category_id='".$ascategory."' OR item.highest_bid<'".$asprice."'";
}
$runsquery = mysqli_query ($connection, $squery)
or die("Error: ".mysqli_error($connection));
